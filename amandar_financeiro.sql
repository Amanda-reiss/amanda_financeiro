...
--
-- Banco de dados: `amandar_financeiro`
--
CREATE DATABASE IF NOT EXISTS `amandar_financeiro` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `amandar_financeiro`;
...
--
DROP TRIGGER IF EXISTS `Tgr_itemvenda_Delete`;
DELIMITER $$
CREATE TRIGGER `Tgr_itemvenda_Delete` AFTER DELETE ON `itemvenda` FOR EACH ROW begin 
update produtos set estoque = estoque - old.Quantidade 
where referencia = old .produto ;
end
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `Tgr_itemvenda_insert`;
DELIMITER $$
CREATE TRIGGER `Tgr_itemvenda_insert` AFTER INSERT ON `itemvenda` FOR EACH ROW begin 
...
(1, 'Amanda Reis Carvalho', '023.146.789-10'),
(2, 'Area Stark', '023.456.789-11');
